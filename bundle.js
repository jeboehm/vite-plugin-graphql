  ...
  "node_modules/apollo-utilities/lib/bundle.cjs.js"(exports) {
    ...
    function removeDirectivesFromDocument(directives, doc) {
      ...
      if (modifiedDoc && filterInPlace(fragmentSpreadsToRemove, function(fs2) {
        return !fragmentSpreadsInUse[fs2.name];
      }).length) {
...
var normalizeGraphQLDocument = (string) => string.replace(/[\s,]+/g, " ").trim();
var addTypenameToDocument = async (doc) => {
  const apolloUtils = await Promise.resolve().then(() => __toESM(require_bundle_cjs()));
  return apolloUtils.addTypenameToDocument(doc);
};
var createPersistentQuery = (sha256Hash) => {
  const version = 1;
  const queryObj = {
    extensions: {
      persistedQuery: {
...
var getCode = async (src) => {
  const graphql = await Promise.resolve().then(() => __toESM(require_graphql2()));
  ...
  const sha256Hash = (0, import_crypto.createHash)("sha256").update(queryAsString).digest("hex");
  if (config.hashMap.enabled) {
    if (!queryHashMap[sha256Hash]) {
      queryHashMap[sha256Hash] = queryAsString;
      await import_fs.promises.writeFile(config.hashMap.path, JSON.stringify(queryHashMap));
    }
  }
